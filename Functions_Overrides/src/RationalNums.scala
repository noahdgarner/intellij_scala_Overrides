...
class Rational (n:Int, d: Int){
  ...
  def add (i: Int): Rational = new Rational(numer+i*denom,denom)
  ...
  def multiply (i: Int): Rational = new Rational(numer * i, denom)
  //overloaded
  def - (that: Rational): Rational = new Rational (numer*that.denom - that.numer*denom,denom*that.denom)
  def subtract (that: Rational): Rational = new Rational (numer*that.denom - that.numer*denom,denom*that.denom)
  def - (i: Int): Rational = new Rational(numer - i * denom, denom)
  def subtract (i: Int): Rational = new Rational(numer - i * denom, denom)
  //overloaded
  def / (that: Rational): Rational = new Rational(numer * that.denom, denom * that.numer)
  def divide (that: Rational): Rational = new Rational(numer * that.denom, denom * that.numer)
  def / (i: Int): Rational = new Rational(numer, denom * i)
  def divide (i: Int): Rational = new Rational(numer, denom * i)
...
println(x.add(y))
println("Test x add y BothRat")
// also works
println(x add y)
//utilize lessThan and max to find the max of the two
println("Test x + y BothRat")
println(x + y) // note that x.+(y) also works, but it is not as readable
println("Test x + 4 1Rat")
println(x + 4) // note that x.+(y) also works, but it is not as readable
println("Test x * y BothRat")
println(x * y)
println("Test x * 4  1Rat")
println(x * 4)
...
x+x*y
(x+x)*y
//notice this implicit def is outside the class. it MUST BE IN THE SCOPE
implicit def intToRational(x: Int) = new Rational(x)
val r = new Rational(4,5)
println("Test Int -> Rational Conversion")
println(2*r)
println(2*r)